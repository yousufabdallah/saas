/*
  # إصلاح مشكلة نوع الإرجاع في دالة المستخدمين

  1. حذف الدوال القديمة
    - حذف get_all_real_users() القديمة
    - حذف get_all_real_stores() القديمة
    - تنظيف الدوال المتضاربة

  2. إنشاء دوال جديدة بأنواع صحيحة
    - استخدام character varying(255) للبريد الإلكتروني
    - توافق كامل مع auth.users
    - معالجة أفضل للأخطاء

  3. تفعيل التسجيل المجاني
...
-- حذف الدوال القديمة المتضاربة
DROP FUNCTION IF EXISTS public.get_all_real_users();
DROP FUNCTION IF EXISTS public.get_all_real_stores();
DROP FUNCTION IF EXISTS public.create_free_store_for_user(text, text, text);
DROP FUNCTION IF EXISTS public.admin_create_store_for_user(text, text, text, text);

-- دالة جديدة للحصول على جميع المستخدمين الحقيقيين مع نوع البيانات الصحيح
...
-- تفعيل إنشاء المتجر التلقائي عند التسجيل
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM auth.users WHERE email = admin_email) THEN
        ...
        -- إضافة الأدمن إلى platform_admins
        INSERT INTO public.platform_admins (user_id) 
        SELECT id FROM auth.users WHERE email = admin_email
        ON CONFLICT (user_id) DO NOTHING;
        
        RAISE NOTICE '✅ تم تأكيد صلاحيات الأدمن';
    ELSE
        RAISE NOTICE '⚠️ الأدمن % غير موجود، يرجى تسجيل الدخول أولاً', admin_email;
    END IF;
    
    RAISE NOTICE '🎯 النظام جاهز: التسجيل المجاني مُفعّل، الدوال الجديدة جاهزة';
END $$;