/*
  # نظام البيانات الحقيقية الكامل

  1. دوال للمستخدمين الحقيقيين
    - جلب جميع المستخدمين من auth.users
    - تمييز بين من لديه متجر ومن ليس لديه
    - معلومات حقيقية كاملة

  2. دوال للمتاجر الحقيقية
    - جلب المتاجر النشطة فقط
    - معلومات أصحاب المتاجر الحقيقية
    - إحصائيات دقيقة
  ...
  4. إدارة شاملة من الأدمن
    - إنشاء متاجر للمستخدمين
...
-- دالة للحصول على جميع المستخدمين الحقيقيين من auth.users
...
BEGIN
  ...
  LEFT JOIN auth.users u ON s.owner_user_id = u.id
  LEFT JOIN (
